...
class Deploy extends Command
{
    ...
    public function handle(Git $git)
    {
        ...
        $this->info("Checking out {$defaultBranch} branch...");
        $repo->checkout($defaultBranch);

        // If the default branch isn't pushed, exit
        $this->info('Checking remote...');
        $this->runProcess('git fetch');
        $ahead = trim($this->runProcess('git rev-list --count --left-only HEAD...@{u}'));
        if ($ahead) {
            $this->error("Local {$defaultBranch} is ahead of remote. Push your changes and try again.");
            return;
        }
